 ...
 *
 * cloud.ts — Capa de nube (Supabase) para el Salón de Estética (molde CyC).
 * Base compartida: validar_licencia / reclamar_tienda / sincronizar_clave_dueno
 * / tl_miembros. Datos del local en la tabla bella_backups (jsonb "datos").
 * Prefijo de la app: bella_*.
 */
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('bella_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('bella_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// ── Sync de datos del local (tabla bella_backups, RLS por membresía) ────
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/bella_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/bella_backups`, {
      method: 'POST',
      headers: {
...
// ── Público (catálogo de servicios) y alta de turno/opinión ───────────
export async function bellaPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('bella_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function bellaAgregarTurno(codigo: string, turno: any): Promise<void> {
  try { await rpc('bella_agregar_turno', { p_codigo: codigo, p_turno: turno }, false); }
  catch (e) { /* noop */ }
}

export async function bellaAgregarResena(codigo: string, comment: any): Promise<void> {
  try { await rpc('bella_agregar_resena', { p_codigo: codigo, p_comment: comment }, false); }
  catch (e) { /* noop */ }
}

export async function bellaVersion(codigo: string): Promise<string> {
